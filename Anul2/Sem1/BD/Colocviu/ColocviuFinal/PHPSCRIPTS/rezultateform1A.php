...
<body>
    <h1>Afisare retete vegetariene</h1>
    <?php
    // creare variabile cu nume scurte
    $dSauN = $_POST['D/N'];
    $nr_matr = trim($dSauN);
    if (!$dSauN) {
        echo '<p class="error-message">Trebuie să introduceți litera D (DA) sau litera N (NU).</p>';
        exit;
    }
    $user = 'root';
    $pass = '';
    ...
    if ($num_results > 0) {
        ...
        for ($i = 0; $i < $num_results; $i++) {
            ...
            echo '<tr>
                    <td>' . ($i + 1) . '</td>
                    <td>' . htmlspecialchars(stripslashes($row['descriere'])) . '</td>
                  </tr>';
        }
        echo '</table>';
    } else {
